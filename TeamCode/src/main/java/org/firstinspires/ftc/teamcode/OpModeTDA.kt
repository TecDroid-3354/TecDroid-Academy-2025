...
@TeleOp(name = "Aaaaa", group = "OpMode")
open class OpModeTDA: CommandOpMode() {
    ...
    lateinit var triggerReader: TriggerReader
    ...
    override fun initialize() {
        ...
        gripper = Gripper(gripperConfig, hardwareMap)

        controller = GamepadEx(gamepad1)

        triggerReader = TriggerReader(controller, GamepadKeys.Trigger.RIGHT_TRIGGER)
            ...
            if (triggerReader.wasJustPressed()) {
                when (arm.currentArmState) {
                    BasketState -> when (arm.currentBasketState) {
                        High -> arm.setPosition(HighBasket)
                        Low -> arm.setPosition(LowBasket)
                    }
                    IntakeState -> arm.setPosition(IntakePose)
                }
            }
        })
    }*/
